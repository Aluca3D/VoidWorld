...
class MobVariationSpawnConfig(
    ...
    private fun loadConfig(root: ConfigurationNode) {
        ...
        for ((key, mobNode) in root.childrenMap()) {
            ...
            mobNode.childrenList().forEachIndexed { index, replacementNode ->
                ...
                val standingOn = parseBlock(
                    replacementNode.node("standingOn").getString("NONE")
                )

                val hasEffect = parseEffect(
                ...
                val tags = replacementNode.node("tags")
                    .childrenList()
    ...
    fun hasVariation(type: EntityType): Boolean =
        getAllVariations(type).isNotEmpty()

    fun getVariation(key: NamespacedKey): RMobVariation? =
        variations[key]?.firstOrNull()
    ...
    fun getKeysForEntity(type: EntityType): List<NamespacedKey> =
        keysByEntity[type].orEmpty()
