...
open class FurnaceRecipeConfig(val recipeGen: RecipeGenerator, val root: ConfigurationNode) : IRecipe {
    ...
    override fun loadRecipes() {
        for ((recipeId, recipeNode) in root.childrenMap()) {
            ...
            val inputChoice: RecipeChoice = when {
                inputNode.isList -> {
                    val materials = inputNode.childrenList().map {
